...
export default function AddProductPage() {
  ...
  const {
    ...
    defaultValues: {
      ...
      stock: 0,
      sku: "",
      category: categories[0]?.name || "Fine Jewelry",
      tags: "",
    },
  });
  ...
  const handleImageUploadPreset = () => {
    // Elegant luxury unsplash preset list
    const luxuryImages = [
    ...
    // Grab a random image not currently present
  ...
  const onSubmit = async (data: ProductFormData) => {
    ...
    setTimeout(() => {
      ...
      toast.success(`${data.name} curated & published live successfully!`);
  ...
  return (
    <div className="space-y-10 max-w-4xl mx-auto">
      ...
      <div className="space-y-4">
        ...
        <div>
          <h1 className="text-3xl font-display text-[var(--color-ivory)]">Curate New Masterpiece</h1>
          <p className="text-zinc-500 font-light text-sm">Fill in details and premium digital assets to publish to the store.</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="space-y-8">
          ...
          <div className="glass-card p-6 rounded-sm space-y-4">
            ...
            {/* Custom URL pasting to fix dummy image upload issue */}
            ...
            <div className="border-t border-zinc-900 pt-3">
              <p className="text-[10px] text-zinc-500 leading-relaxed font-light">
                ...
                At least <strong className="text-[var(--color-gold)]">3 premium curation images</strong> are required to publish this product live. 
          ...
          <Button type="submit" size="lg" disabled={isSubmitting} className="w-full py-4 text-xs font-bold tracking-widest uppercase cursor-pointer">
            {isSubmitting ? "PUBLISHING TO STORE..." : "PUBLISH TO LIVE STORE"}
          </Button>
        </div>
      </form>
    </div>
